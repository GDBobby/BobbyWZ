...
namespace MapleLib {
	namespace WzLib {
		namespace WzProperties {
			...
			protected:
				WzValueProperty() { }
				WzValueProperty(std::wstring name) : name{ name } {}
				WzValueProperty(std::wstring name, T value) : name{ name }, val{ value } { }
			public:
				std::wstring name;
				T val;
				WzObject parent;
			...
			public:
				...
				float GetDouble() override { return static_cast<double>(val); }
